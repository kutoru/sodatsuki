...
import { NoteMemoWrapper } from "./NoteMemoWrapper";
...
const renderType = "original" as "original" | "windowing" | "memoized";
(window as any).start = 0;
...
export const LeftPanel = ({ leftPanel, leftResize, blurFilter }: Props) => {
  ...
  const loadDeck = (deck: string) => {
    ...
    invoke<DeckState | undefined>("anki_fetch_deck", {
      deck,
      ...
      .then((deck) => {
        updateDigitWidth(deck?.notes.length ?? 0);
        setDeck(deck);
      })
      .catch(handleError())
      .finally(() => {
  ...
  const updateDigitWidth = (notesLength: number) => {
  ...
  useEffect(() => {
    console.log("deck", Date.now() - (window as any).start);
  }, [deck]);

  useEffect(() => {
    console.log("note", Date.now() - (window as any).start);
  }, [currentNote]);
  ...
  return (
    <>
      ...
      >
        ...
        {renderType === "windowing" && deck?.notes && (
          <List
            className="pb-2 slim-scrollbar"
            rowComponent={NoteWrapper}
